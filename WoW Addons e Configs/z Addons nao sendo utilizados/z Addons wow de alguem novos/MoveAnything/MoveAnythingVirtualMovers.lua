...
MovAny.lVirtualMovers = {
	...
	RaidUnitFramesMover = {
		...
		OnLoad = function(self)
			hooksecurefunc("CompactRaidFrameManager_SetSetting", function(setting, value, skip)
				if skip ~= "MASkip" and setting == "Locked" and not IsShiftKeyDown() then
	...
	RaidUnitFramesManagerMover = {
		...
		dontLock = true,
		OnLoad = function(self)
			local b = CreateFrame("Button", "MACompactRaidFrameManagerToggleButton", UIParent, nil, "MADontHook")--"GameMenuButtonTemplate"
			b:SetSize(16, 64)
		...
		OnMAPreReset = function(self, opt)
			MovAny:ResetPosition(self.man, opt, true)
			self.button:Hide()
			self.man = nil
		end,
	},
