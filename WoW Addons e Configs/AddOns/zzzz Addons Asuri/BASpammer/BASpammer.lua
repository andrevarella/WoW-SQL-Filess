...
function BASpammer_FillTable()
	for i=1,10 do
		BASpammerDB.Pattern[i] = "empty"
	end
	BASpammerDB.CheckedPattern = 1
	BASpammerDB.Channel = 1
	BASpammerDB.Interval = 30
...
function BASpammer:OnMouseDown(self, arg1)
	...
	BASpammerSettingTextPatternEditBox:SetText("Sample " .. tostring(BASpammerDB.CheckedPattern))
	...
	BASpammerSettingChanelEditBox:SetText("Channel " .. BASpammerDB.Channel)
...
function BASpammer:OnUpdate() -- собственно сам спам!
	if BASpammerDB.Tumbler then
		if time() - BASpammerDB.LastTimeSpam >= tonumber(BASpammerDB.Interval) then
		...
		BASpammerSettingTaximeterText:SetText(tostring(tonumber(BASpammerDB.Interval) - (time()-BASpammerDB.LastTimeSpam)))
	end
end
...
function TextPattern_OnClick(arg1)
	BASpammerSettingTextPatternEditBox:SetText("Sample " .. tostring(arg1))
...
function BASpammerChannelsDropdown_OnClick(arg1)
	BASpammerSettingChanelEditBox:SetText("Channel " .. arg1)
...
TextPatternDropdown.initialize = function(self, level)
	...
	for i =1,10 do
		...
		info.tooltipTitle = tostring("Sample " .. i)
...
function BASpammerSettingTextBox_OnTextChanged()
	...
	BASpammerSettingTextSymbols:SetText("Symbols: " .. tostring(BASpammerSettingTextBox:GetNumLetters()).."/255")	
end
...
function BASpammer_OnEvent(BASpammer_self, BASpammer_event, BASpammer_arg1, ...)
	if( BASpammer_event == "ADDON_LOADED" and BASpammer_arg1 == "BASpammer") then
		print("Addon " .. BASpammer_arg1 .. " loaded.");
...
function BASpammerSettingStartButton_OnClick()
	if BASpammerDB.Interval < 30 then
		BASpammerDB.Interval = 30
		BASpammerSettingIntervalEditBox:SetText("30")
	end
...
function BASpammer:OnEnter()
	if BASpammerDB.Tumbler then
		...
		BASpammerTooltip:AddLine("Spam is coming!",0.77, 0.12, 0.23)
		BASpammerTooltip:AddLine("Channel: " .. "|cffffffff" .. tostring(BASpammerDB.Channel) .."|r")
		BASpammerTooltip:AddLine("Interval: " .. "|cffffffff" .. tostring(BASpammerDB.Interval) .." sec|r")
		BASpammerTooltip:AddLine("Text:")
