...
function LoseControl:UNIT_AURA(unitId) -- fired when a (de)buff is gained/lost
	...
	for i = 1, 40 do
		...
		local debufType = abilities[name]
		if LoseControlDB.tracking[debufType] and (priorities[debufType] < priority or priorities[debufType] == priority and expirationTime > maxExpirationTime) then
			priority = priorities[debufType]
