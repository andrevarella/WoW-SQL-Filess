...
local function GetSnowfallAnimation()
	...
	-- Start by making the animation texture 2.5x the size of the button
	local scale1 = animationGroup:CreateAnimation("Scale");
	scale1:SetScale(2.5, 2.5);
	scale1:SetDuration(0);
	scale1:SetOrder(1);

	-- Over 1.0 seconds, scale the animation texture down to zero size
	local scale2 = animationGroup:CreateAnimation("Scale");
	scale2:SetScale(0, 0);
	scale2:SetDuration(1.0);
	scale2:SetOrder(2);

	-- Over 1.0 seconds, rotate the animation texture counter-clockwise by 90 degrees
	local rotation2 = animationGroup:CreateAnimation("Rotation");
	rotation2:SetDegrees(90);
	rotation2:SetDuration(1.0);
	rotation2:SetOrder(2);

	return frame, animationGroup
end

local snowfallFrame, snowfallAnimation = GetSnowfallAnimation()
...
function Gladius:AuraGain(unit, name, icon, expirationTime, priority)
	...
	if unit and lastAura[unit] ~= name then

		lastAura[unit] = name

		-- Place the animation on top of the button
		snowfallFrame:SetFrameStrata(aura:GetFrameStrata());
		snowfallFrame:SetFrameLevel(aura:GetFrameLevel() + 10);
		snowfallFrame:SetAllPoints(aura.icon);

		-- Play the animation from the beginning
		snowfallAnimation:Stop();
		snowfallAnimation:Play();
	...
	if (arenaUnits[unit] == "playerUnit" and db.targetIcon) then
      local button = self.buttons[unit]
		...
		if (targetClass ~= nil) then
			targetIcon:SetTexture("Interface\\glues\\CHARACTERCREATE\\UI-CharacterCreate-Classes2")
...
function Gladius:UpdateUnit(unit)
	...
	if (UnitExists(unit)) then
      if (arenaUnits[unit] == "playerUnit" or (arenaUnits[unit] ~= "playerUnit" and db.showPets)) then
		...
		--class icon
		button.classIcon:SetTexture("Interface\\glues\\CHARACTERCREATE\\UI-CharacterCreate-Classes2")
...
function Gladius:Test()
	...
	for i=1, Gladius.currentBracket do
		...
		button.targetIcon:SetTexture("Interface\\glues\\CHARACTERCREATE\\UI-CharacterCreate-Classes2")
