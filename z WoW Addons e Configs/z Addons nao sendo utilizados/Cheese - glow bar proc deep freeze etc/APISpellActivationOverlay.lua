	...
	--	print(globalID)
		...
		elseif ( actionType == "macro" ) then
			...
			if(name) then 
				...
				spellID = string.sub(spellLink, 19, string.find(spellLink, "%[") -3)
				spellID = tonumber(spellID)
			end
				ChangeAction(action, tonumber(spellID));
				--print(action)
			--	print(spellID)
		else
		...
		for action=1, NUM_ACTION_BUTTONS do
			...
			elseif ( actionType == "macro" ) then
				...
				if ( actionButtons[action] ~= spellID ) then
					ChangeAction(action, spellID);
				end
			else
				if ( actionButtons[action] ) then
					ChangeAction(action, nil);
	...
	do
		for action=1, NUM_ACTION_BUTTONS do
			...
			if(name) then 
				...
				--print(action, spellID)
				SetAction(action, spellID);
			end
		end
	end
